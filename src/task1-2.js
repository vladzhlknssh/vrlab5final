...
let coneMaterial, coneMaterialNoTexture;
...
function init() {
  ...
  const glassTexture = textureLoader.load(
    "https://preview.redd.it/sda15hvzgpm41.png?auto=webp&s=dffe6083f4f77ad9203aa46c1233604d357d536b"
  );
  ...
  const lavaTexture = textureLoader.load(
    "https://images.pexels.com/photos/2832432/pexels-photo-2832432.png?cs=srgb&dl=pexels-anniroenkae-2832432.jpg&fm=jpg"
  );

  // 1. Cone
  const coneGeometry = new THREE.ConeGeometry(1.5, 2);
  coneMaterial = new THREE.MeshPhysicalMaterial({
    map: glassTexture,
    transparent: true,
    opacity: 0.7,
    roughness: 0.5,
  ...
  coneMaterialNoTexture = new THREE.MeshPhysicalMaterial({
        color: 0x111111,
        emissive: 0x00ffcc,
  ...
  coneMesh.position.set(-4, 0, -5);
  scene.add(coneMesh);

  // 2. Тетраедр (Tetrahedron)
  const tetrahedronGeometry = new THREE.TetrahedronGeometry(1.6);
...
function toggleTextures() {
  ...
  coneMesh.material = texturesEnabled
    ? coneMaterial
    : coneMaterialNoTexture;
...
function animateObjects(timestamp) {
  ...
  if (pulseMoveEnabled) {
    ...
    ringMesh.geometry = new THREE.RingGeometry(0.25, 0.15, 50, 16);
  }
  if (colorEmitEnabled) {
